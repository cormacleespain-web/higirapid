...
import { getAllServicesAdmin, type ServiceAdminRow } from "@/lib/admin-queries";
...
export default async function AdminServicesPage() {
  ...
  let rows: ServiceAdminRow[] = [];
  let fetchError: string | null = null;
  if (dbOk) {
    const r = await getAllServicesAdmin();
    rows = r.rows;
    fetchError = r.error;
  }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            Homepage carousel, Our Services page, and booking flows use this list. Edit copy, image, icon, optional from
            price, and languages per service. Drag rows to set order everywhere.
          </p>
        </div>
        <Link
      ...
      {fetchError ? (
        <p className="mt-6 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900">{fetchError}</p>
      ) : null}
